...
export default function ReadENS(props: Props) {
    ...
    const [ensRegistry, setENSRegistry] = useState<string>("0xc67a1473D87b669Bb5658Eb4771BBC3b9e189a18")
    ...
    useEffect(() => {
        ...
        const offResoContract: Contract = new ethers.Contract(offchainResolver, abi, provider)
        offResoContract
            .url()
            .then((url: string) => {
                setGatewayURL(url)
    ...
    async function resolver(event: React.FormEvent) {
        ...
        const providerWithENS = new ethers.BrowserProvider(window.ethereum, {
    ...
    return (
        <div>
            <form onSubmit={resolver}>
                <form>
                    <label htmlFor="domainname">Please Enter A Domain Name Here:</label>
                    <input
                        id="domainname"
                        type="text"
                        required
                    ...
                    />
                    <label htmlFor="resolve" color={currentAccount ? "black" : "red"}>
                        {currentAccount ? "Please Click Here To Resolve:" : "Disabled To Resolve! Need Connect To MetaMask First!"}
                    </label>
                    <button type="submit" disabled={!currentAccount}>
                        Resolve
                    </button>
                    <label htmlFor="offchainresolver">OffchainResolver Contract Address (Get From ENSRegistry Contract):</label>
                    <input id="offchainresolver" type="text" value={offchainResolver} disabled />
                    <label htmlFor="gateway">Gateway Server URL (Get From OffchainResolver Contract):</label>
                    <input id="gateway" type="text" value={gatewayURL} disabled />
                    <label htmlFor="resolveddata">Resolved Data:</label>
                    <input
                        id="resolveddata"
                        type="text"
                        value={resolvedData !== null ? resolvedData : "[Error] No match data (from " + gatewayURL + ")"}
                        disabled
                    />
                    <label htmlFor="errormessage" color={errorMessage || resolvedData === null ? "red" : "black"}>
